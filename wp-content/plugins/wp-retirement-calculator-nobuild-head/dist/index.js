...
function Currency({ value }) { const s = Number(value || 0).toLocaleString(undefined, { maximumFractionDigits: 0 }); return html`<span>$${s}</span>`; }
function NumberInput({ label, value, setValue, step = "1", min = "0", max, suffix }) {
  return html`<label class="field"><span>${label}</span>
    <input type="number" value=${value} step=${step} min=${min} max=${max} onInput=${(e)=>setValue(e.currentTarget.value)} />
    ${suffix ? html`<span class="suffix">${suffix}</span>` : null}</label>`;
}
function PctInput(p){ return html`<${NumberInput} ...${p} step="0.1" min="0" max="100" suffix="%" />`; }
function Section({ title, children }) { return html`<section class="card"><h3>${title}</h3><div class="grid">${children}</div></section>`; }
...
function App(){
  ...
  const payload = useMemo(()=> ({
    age: num(age), retireAge: num(retireAge), salary: num(salary), currentBalance: num(currentBalance),
    sgRate: num(sgRate)/100, volPreTaxPct: num(volPreTaxPct)/100, volAfterTaxAnnual: num(volAfterTaxAnnual),
    salaryGrowth: num(salaryGrowth)/100, returnNominal: num(returnNominal)/100,
    earningsTax: num(earningsTax)/100, contribTax: num(contribTax)/100,
    feePct: num(feePct)/100, feeFixed: num(feeFixed), inflation: num(inflation)/100,
    longevityAge: num(longevityAge), agePensionPa: num(agePensionPa),
  ...
  async function calculate(){
    ...
    try{
      const res = await fetch(WPRC_CFG.restUrl, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify(payload) });
  ...
  const balanceChartData = useMemo(()=>{
    ...
    return { labels, datasets: [{ label: "Projected Super Balance", data }] };
  }, [result]);

  useEffect(()=>{
    const canvas = document.getElementById("wprc-chart");
    if(!canvas || !balanceChartData) return;
    const chart = new Chart(canvas, {
      type:"line", data: balanceChartData,
      options:{ responsive:true, maintainAspectRatio:false, scales:{ y:{ ticks:{ callback:(v)=>'$'+Number(v).toLocaleString() } } } }
    });
          ...
          <section class="card">
            ...
            ${result ? html`
              <div class="results">
                <div class="kpis">
                  <div class="kpi">
                    <span class="label">Projected balance at ${result.atRetirement.age}</span>
                    <strong><${Currency} value=${result.atRetirement.balance} /></strong>
                  </div>
                  <div class="kpi">
                    <span class="label">Annual income from super (today's $)</span>
                    <strong><${Currency} value=${result.income.annualSuperIncomeToday} /></strong>
                  </div>
                  <div class="kpi">
                    <span class="label">Expected Age Pension (today's $)</span>
                    <strong><${Currency} value=${result.income.annualAgePensionToday} /></strong>
                  </div>
                  <div class="kpi">
                    <span class="label">Estimated total annual income</span>
            ...
            : html`<p>Enter your details and click "Update results".</p>`}
          </section>
        </div>
      </div>
    </div>
...
function mount(){ const el = document.getElementById("retcalc-root"); if(!el) return; render(h(App), el); }
