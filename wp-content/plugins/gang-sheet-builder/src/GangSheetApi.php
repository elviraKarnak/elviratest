...
class GangSheetApi
{
    ...
    public function updateProduct($request)
    {
        try {
            ...
            // Get current attributes and post meta
            $attributes = $product->get_attributes();
            $product_attributes_meta = get_post_meta($product_id, '_product_attributes', true);
            if (!is_array($product_attributes_meta)) {
                $product_attributes_meta = [];
            }

            $size_attribute_key = 'size'; // lowercase key for custom attribute
            ...
            if (!empty($data['variants'])) {
                ...
                $current_size_values = [];
                if (isset($attributes[$size_attribute_key])) {
                    $current_size_values = $attributes[$size_attribute_key]->get_options();
                }

                // Get new variant titles
                $new_size_values = array_map(fn($v) => $v['title'], $variants);

                $size_values = !empty($data['product_title'])
                    ? $new_size_values
                    : array_unique(array_merge($current_size_values, $new_size_values));

                // Update WC_Product_Attribute
                if (isset($attributes[$size_attribute_key])) {
                    $attributes[$size_attribute_key]->set_options($size_values);
                } else {
                    $attribute = new \WC_Product_Attribute();
                    $attribute->set_name("Size"); // display name
                    $attribute->set_options($size_values);
                    $attribute->set_visible(true);
                    $attribute->set_variation(true);
                    $attributes[$size_attribute_key] = $attribute;
                }

                // Update post meta for admin tab
                $product_attributes_meta[$size_attribute_key] = [
                    'name'         => 'Size',
                    'value'        => implode(' | ', $size_values),
                    'position'     => 0,
                    'is_visible'   => 1,
                    'is_variation' => 1,
                    'is_taxonomy'  => 0
            ...
            } elseif (empty($data['variants']) && !empty($data['artBoardType']) && ($data['artBoardType'] == 7)) {

                $existingVariantIds = $product->get_children();
                foreach ($existingVariantIds as $variant_id) {
                    $variation = new \WC_Product_Variation($variant_id);
                    if ($variation->exists()) {
                        $variation->delete();
                    }
                ...
                $variation->set_parent_id($product_id);
                $variation->set_regular_price(0.01);
                $variation->set_price(0.01);
                $variation->set_attributes(['attribute_size' => 'Small']);

                $variation->save();

                update_post_meta($variation->get_id(), '_title', 'Small');
                update_post_meta($variation->get_id(), '_g_width', 2);
                update_post_meta($variation->get_id(), '_g_height', 2);
                update_post_meta($variation->get_id(), '_price', 0.01);
                update_post_meta($variation->get_id(), '_unit', 'in');
                update_post_meta($variation->get_id(), '_max_allowed_file_count', -1);
            }

            // Update product title if provided
            if (!empty($data['product_title'])) {
                $product->set_name($data['product_title']);
            }
