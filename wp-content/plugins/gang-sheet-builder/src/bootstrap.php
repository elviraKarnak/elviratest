...
if (!function_exists('delete_gs_access_token')) {
    function delete_gs_access_token()
    {
        return delete_option(KEY_GANG_SHEET_ACCESS_TOKEN);
    }
}
...
if (!function_exists('delete_gs_shop_uuid')) {
    function delete_gs_shop_uuid()
    {
        return delete_option(KEY_GANG_SHEET_SHOP_UUID);
    }
}
...
if (!function_exists('delete_gs_shop_slug')) {
    function delete_gs_shop_slug()
    {
        return delete_option(KEY_GANG_SHEET_SHOP_SLUG);
    }
}
...
if (!function_exists('gs_shop_register')) {
    function gs_shop_register()
    {
        ...
        if (empty($options['btn_text'])) {
            $options['btn_text'] = 'Build your own gang sheet';
        }

        if (empty($options['btn_bg_color'])) {
            $options['btn_bg_color'] = '#000000';
        }

        if (empty($options['btn_fg_color'])) {
            $options['btn_fg_color'] = '#ffffff';
        }

        set_gs_options($options);
...
if (!function_exists('gs_update_store_currency')) {
    function gs_update_store_currency()
    {
        $currency = get_woocommerce_currency();
        $currencySymbol = get_woocommerce_currency_symbol();
        $currencySymbol = html_entity_decode($currencySymbol, ENT_QUOTES, 'UTF-8');
        ...
        gang_sheet_api_call('POST', 'currency', $post_data);
    }
}
